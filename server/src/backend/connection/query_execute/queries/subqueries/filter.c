...
static row_likedlist_t *complex_condition(db_t *db,
    ...
    if (root->r != NULL && ((struct filter_condition_ast *) root)->logic != -1) {
        ...
        row_likedlist_t *left_filtered = simple_condition(db, left, rll, schema, resp);
        row_likedlist_t *right_filtered = complex_condition(db, right, rll, schema, resp);
        ...
        struct filter_expr_ast *left = (struct filter_expr_ast *) root->l;
        result_list = simple_condition(db, left, rll, schema, resp);
        return result_list;
    }
}

row_likedlist_t *
filter_exec(db_t *db, struct ast *root, row_likedlist_t *rll, schema_t *schema, struct response *resp) {
    ...
    if (condition_ast_ptr->r != NULL && condition_ast_ptr->logic != -1) {
        result_list = complex_condition(db, condition_ast_ptr, rll, schema, resp);
    } else {
        result_list = simple_condition(db, (struct filter_expr_ast *) condition_ast_ptr->l, rll, schema, resp);
    }
